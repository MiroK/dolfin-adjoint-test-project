from delaunay_mesh import build_mesh
...
if __name__ == '__main__':
    ...
    # Want to mesh a circle (cx, cy) with radius ...
    cx = 0
    cy = 0
    radius = 1
    resolution = 0.5

    # ... and the following points inside
    r = np.random.rand(10)
    th = 2*np.pi*np.random.rand(10)
    embed_pts = np.c_[cx + r*np.cos(th), cy + r*np.sin(th)]
    ...
    # Points in surface we want to keep track of
    point_tags = [factory.addPoint(*point, z=0) for point in embed_pts]
    ...
    nnz_idx, = np.where(vertex_values > 0)
    for idx in nnz_idx:
        gmsh_tag = vertex_values[idx]
        x = mesh_coordinates[idx]
        y = embed_pts[int(gmsh_tag-1)]

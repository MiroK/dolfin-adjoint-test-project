...
def read_pO2_from_file(filename):
    ...
    mesh = RectangleMesh(Point(0, 0), Point(4, 3), Nx-1, Ny-1)
    ...
    def boundary1(x, on_boundary):
        r = np.sqrt((x[0]-1)**2 + (x[1]-1.1)**2)
        b = (r <= R_ves)
        return b
    bc1 = DirichletBC(V, p_ves[0], boundary1, "pointwise")
    
    def boundary2(x, on_boundary):
        r = np.sqrt((x[0]-2.8)**2 + (x[1]-0.8)**2)
        b = (r <= R_ves)
        return b
    bc2 = DirichletBC(V, p_ves[1], boundary2, "pointwise")
    
    def boundary3(x, on_boundary):
        r = np.sqrt((x[0]-3)**2 + (x[1]-2)**2)
        b = (r <= R_ves)
        return b
    bc3 = DirichletBC(V, p_ves[2], boundary3, "pointwise")
...
def create_synthetic_pO2_data(mesh, hole, sigma):
    ...
    p_ves = [80/(M_star*R_star**2), 70/(M_star*R_star**2), 50/(M_star*R_star**2)]  # pO2 at vessel wall
    M = 1
...
if __name__ == "__main__":
    ...
    Nx_ = np.ceil(4/d)
    Ny_ = np.ceil(3/d)
    x = np.linspace(0, 4, Nx_)
    y = np.linspace(0, 3, Ny_)
    Nx = len(x)
    Ny = len(y)
    mesh = RectangleMesh(Point(0, 0), Point(4, 3), Nx-1, Ny-1)
    #plot(mesh)
    ...
    np.savez('synthetic_data/pO2_data_manyvessels', \
    p=p, p_noisy=p_noisy, 
    x=x, y=y, d=d, p_ves=p_ves, R_ves=R_ves)

    filename = 'synthetic_data/pO2_data_manyvessels.npz'
...
#    plt.show()
    ...
    np.savez('results/results_manyvessels.npz', p_exact=p_exact, p_noisy=p_noisy, p_opt=p_opt, M_opt=M_opt, x=x, y=y, alpha=alpha)
